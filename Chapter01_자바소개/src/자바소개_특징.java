 ...
 * 		================================
 * 		 데이터가 저장 : 메모리에 저장 => RAM(프로그램 종료시에 사라잠)
 * 		             => 파일 / RDMS(오라클)
 * 2. 사용자가 요청 따라 처리 (데이터 처리)
 * 		=> 연산자 / 제어문
 * 3. 여러개를 동시에 저장
 * 		=> 같은 데이터만 저장 (정수, 실수....) 배열
 * 		=> 다른 데이터 저장 : 클래스 (*******)
 ...
 *		=> 무료 / 유료
 *	 => File => new => Java Project 선택 => 프로젝트 폴더 생성
 *	 	=> src안에 class 생성 후 사용
 *				  =====
 *				  ㅣ 프로그램의 시작점 => main
 *
 ...
 *  	public static void main(String[] arg){ => 매소드 시작
 *  	==> 기능 수행 => 소스 코드
 *  	} => 메소드 종료
 *  } => 클래스 종료
 *  
